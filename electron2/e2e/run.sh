...
# ============================================================================
# ⚠️ 已知缺口：这套用例**没有夹具层**，它依赖开发机上那个自然长起来的库。
#
# 后果（实测过两次）：
#   · 连跑几轮之后开始出现无规律的红 —— 用例用固定 id 插桩、累积的记录让
#     「今天已照顾过这片花瓣」变成 true、断言里的条数一轮涨一条（74→75→76→77）。
#     **一个不幂等的测试套件，迟早会拿自己的残渣去判产品的罪。**
#   · 而它红得毫无规律，下一个人只会以为是产品坏了。
#
# 试过 `--user-data-dir` 给它一个一次性 userData —— 架构上是对的（这套用例
# 本身是破坏性的，有阶段直接调 dbClearAll，本就不该对着真实库跑），
# **但全新库里没有任何历史，大量用例的前提直接不成立**
# （「最近的记录 ≥3 条」「这一周有记录」「满 30 天」…），失败面反而更大。
#
# ⇒ 正确的下一步是**补一层夹具**（跑前灌一份已知的花园），不是删库也不是换库。
#   在那之前：单条用例自己负责幂等（见 operation-test.mjs 里 'aha-floor-' 那段），
#   阶段自己负责收尾（见阶段 10.10 末尾）。
#   记在 v3.7 归档的行动清单里。
